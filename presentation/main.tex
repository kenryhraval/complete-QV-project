...
\begin{frame}{Goal of the presentation}
    \begin{columns}[T,totalwidth=\textwidth]
        \column{0.4\textwidth}
        We study how Quantum Volume depends on \emph{where} a circuit is placed on a chip, not only on the chip as a whole.
...
\begin{frame}{NISQ computers}
    ...
    \begin{block}{Takeaway}
        In the NISQ era, \emph{where} a circuit is placed on a chip can matter almost as much as \emph{what} circuit is run.
    \end{block}
\end{frame}
...
\begin{frame}{Noisy simulators}
    \begin{itemize}
        \item A noisy simulator reproduces hardware-relevant errors using a calibrated noise model.
        \item It enables controlled comparison of many circuit placements under identical conditions.
        \item In this project, calibration data and the coupling graph define the simulated device model.
    \end{itemize}

    \begin{block}{Why simulation?}
        It is much easier to scan hundreds of connected subsets reproducibly on a simulator than on a live device.
    \end{block}
\end{frame}
...
\begin{frame}{Results (1)}
    ...
    \textbf{Unoptimized circuits}
    \vspace{0.3em}

    \input{../results/qv_summary_stats.tex}

    \vspace{0.8em}

    \textbf{Optimized circuits}
    \vspace{0.3em}

    \input{../results/qv_summary_stats_optimised.tex}
...
\begin{frame}{Results (2)}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.88\linewidth,keepaspectratio]{../results/optimisation_comparison.png}
        \caption{Without and with optimisation comparison for $n=6$}
    \end{figure}
\end{frame}
...
\begin{frame}{Results (3)}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth,keepaspectratio]{../results/aggregated_qv_grid.png}
        \caption{Aggregated QV margin across subset sizes}
    \end{figure}
\end{frame}
...
\begin{frame}{Results (4)}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth,keepaspectratio]{../results/success_fraction_grid.png}
        \caption{Success fraction of QV circuits across subset sizes}
    \end{figure}
\end{frame}
...
\begin{frame}{Results (5)}
    \begin{figure}
        ...
        \caption{Best and worst performing subsets across subset sizes}
    \end{figure}
\end{frame}
